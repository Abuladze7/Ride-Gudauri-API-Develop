exports.skiSchoolIndividualSessionBookingsTemplate = (body) => {
  ...
  const cost = () => {
    if (currency.includes("$")) return Number(currency.replace("$", ""));

    return Number(currency.replace("₾", ""));
  };

  const offTotal = (cost() * 15) / 100;
        ...
        <div class="table-wrapper">
          ...
          <div class="total">
            ...
            <div class="currencies">
              <p><strong>Total amount (in GEL): </strong> ${currency}</p>
              <p><strong>Total amount (in USD): </strong> ${currency}</p>
            </div>
          </div>
        </div>
      </div>
