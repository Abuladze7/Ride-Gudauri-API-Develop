exports.skiLessonBookingTemplate = (body) => {
  ...
  const formatedStartDate = new Date(fromDate.split("/").reverse().join("-"));
  const formatedEndDate = new Date(toDate.split("/").reverse().join("-"));

  const totalDays =
    (formatedEndDate - formatedStartDate) / (1000 * 60 * 60 * 24) + 1;

  const totalHours = () => {
    if (hours.toLowerCase() === "full day") return 8 * totalDays;
